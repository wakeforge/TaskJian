...
// 默认标签（PDF 指定）+ 样例中使用的 T1（用户自定义标签）
const TAGS = ['P0', 'P1', 'S1', 'S2', 'T0', 'T1', 'ED'];
...
describe('parseTaskText — 行级语法', () => {
  it('1. 单个待办任务：-T0 测试任务', () => {
    const r = parseTaskText('-T0 测试任务', TAGS);
    expect(r.tasks).toHaveLength(1);
    const t = r.tasks[0];
    expect(t.status).toBe('todo');
    expect(t.tags).toEqual(['T0']);
    expect(t.title).toBe('测试任务');
    expect(t.parentId).toBeNull();
    expect(r.rootOrder).toEqual([t.id]);
  });

  it('2. 进行中任务带编号：=S2 101: 国库支付', () => {
    const r = parseTaskText('=S2 101: 国库支付', TAGS);
    const t = r.tasks[0];
    expect(t.status).toBe('progress');
    expect(t.tags).toEqual(['S2']);
    expect(t.code).toBe('101');
    expect(t.title).toBe('国库支付');
  });

  it('3. 已完成任务：*ED 完成', () => {
    const r = parseTaskText('*ED 完成', TAGS);
    const t = r.tasks[0];
    expect(t.status).toBe('done');
    expect(t.tags).toEqual(['ED']);
    expect(t.title).toBe('完成');
  });

  it('4. 受阻任务：▲T1 阻塞', () => {
    const r = parseTaskText('▲T1 阻塞', TAGS);
    const t = r.tasks[0];
  ...
  it('9. 多标签：-T0 ED 测试', () => {
    const r = parseTaskText('-T0 ED 测试', TAGS);
    const t = r.tasks[0];
    expect(t.tags).toEqual(['T0', 'ED']);
  ...
  it('额外. ✓ 前缀 → done（替代完成前缀）', () => {
    const r = parseTaskText('✓ED 已完成', TAGS);
    expect(r.tasks[0].status).toBe('done');
    expect(r.tasks[0].tags).toEqual(['ED']);
  });
...
└─ *ED 其他流水凭证`;
    ...
    const serialized = serializeTasks(pr1.tasks, pr1.groups, pr1.rootOrder);
    const pr2 = parseTaskText(serialized, TAGS);
